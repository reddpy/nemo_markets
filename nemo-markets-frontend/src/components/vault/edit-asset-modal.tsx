...
const EditAssetModal = ({
  ...
  const form = useForm<z.infer<typeof assetSchema>>({
    ...
    defaultValues: {
      ...
      asset_category: asset_obj.category,
      asset_stage: asset_obj.stage,
      asset_description: asset_obj.description,
      asset_price: asset_obj.asking_price,
    },
  });

  const onSubmit = async (values: z.infer<typeof assetSchema>) => {
    const updated_payload = { id: asset_obj.id, ...values };
    const response = await update_ip(updated_payload);
    if (response) {
      form.reset();
      successToastFunc(
        values.asset_name,
